...
def eulerNiejawny():
    ...
    or wfInput.get()=="" or wgInput.get()==""):
        ...
        try:
            ...
            x = float(inputX.get())
            fy1 = float(eval(fy1Input.get()))
            fy2 = float(eval(fy2Input.get()))
            gy1 = float(eval(gy1Input.get()))
            gy2 = float(eval(gy2Input.get()))
...
#------GUI------
Label(root,text="f(x) = ").grid(row=0,column=0)
fy1Input = Entry(root,bg="#FDFDFD")
fy1Input.insert(END,'x')
...
Label(root,text="g(x) = ").grid(row=1,column=0)
